...
const KanbanBoard: React.FC<KanbanBoardProps> = ({ 
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // Prevent drag scrolling when interacting with cards or buttons
    if (target.closest('button') || target.closest('[draggable="true"]')) {
      return;
    }
    
    // Prevent drag scrolling if clicking on the vertical scrollbar of a column
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-white dark:bg-[#161618]">
      {/* Contenedor principal de la pizarra con scroll horizontal */}
      <div 
