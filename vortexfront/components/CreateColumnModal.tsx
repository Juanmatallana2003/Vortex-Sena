...
interface CreateColumnModalProps {
  onClose: () => void;
  onSave: (newColumn: Omit<Column, 'cards'>) => void;
}
...
const CreateColumnModal: React.FC<CreateColumnModalProps> = ({ onClose, onSave }) => {
  const [title, setTitle] = useState('');
  const [keyword, setKeyword] = useState('');
  const [selectedColor, setSelectedColor] = useState('bg-neutral-500');
  ...
  const handleSave = () => {
    if (title.trim()) {
      onSave({
        ...
        title: title.trim(),
        keyword: keyword.trim() || title.toLowerCase(),
        color: selectedColor,
  ...
  return (
    ...
    >
      <div
        className="bg-white dark:bg-[#1c1c1f] border border-neutral-200 dark:border-neutral-800 rounded-lg w-full max-w-sm flex flex-col animate-scale-in shadow-xl"
        onClick={handleModalContentClick}
      >
        <header className="flex items-center justify-between p-4 border-b border-neutral-200 dark:border-neutral-800">
          <h1 id="create-column-title" className="text-lg font-medium text-neutral-900 dark:text-neutral-100">Add Column</h1>
          <button
            onClick={onClose}
          ...
          <div className="pt-2 border-t border-neutral-200 dark:border-neutral-800 mt-4">
            <label className="flex items-start gap-3 cursor-pointer group p-2 hover:bg-gray-100 dark:hover:bg-neutral-800/50 rounded-lg transition-colors">
               ...
               <div className="flex flex-col flex-1">
                   <span className="text-sm font-medium text-neutral-900 dark:text-neutral-200 group-hover:text-black dark:group-hover:text-white transition-colors">
                       Mark as Done
                   </span>
